...
export const devenvRead = tool({
  ...
  async execute(args) {
    ...
    if (args.pattern) {
      ...
      const result = ptyManager.search(args.id, regex, offset, limit);
      if (!result) {
      ...
      if (result.matches.length === 0) {
        return [
          `<devenv_output id="${args.id}" status="${session.status}" pattern="${args.pattern}">`,
      ...
      output.push(`</devenv_output>`);

      return output.join("\n");
    }

    const result = ptyManager.read(args.id, offset, limit);
    if (!result) {
      throw new Error(`PTY session '${args.id}' not found.`);
    }
    ...
    if (result.lines.length === 0) {
      return [
        `<devenv_output id="${args.id}" status="${session.status}">`,
